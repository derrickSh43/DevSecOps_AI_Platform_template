...
async def nlp_analyze(input: TextInput):
    NLP_SERVICE_URL = os.getenv("NLP_SERVICE_URL", "http://nlp-assistant:8000")
    try:
        response = requests.post(f"{NLP_SERVICE_URL}/analyze", json={"text": input.text})
        return response.json()
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"NLP call failed: {str(e)}")
